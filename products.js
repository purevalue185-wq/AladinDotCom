...
function renderProducts() {
  ...
  if (searchQuery) { const q = searchQuery.toLowerCase(); filtered = filtered.filter(p => p.title.toLowerCase().includes(q) || p.category.toLowerCase().includes(q)); }
  ...
  container.innerHTML = filtered.map(p => {
    ...
    <div class="deal-card" onclick="goToDetail('${p.id}')">
      <div class="deal-img">${imgHTML}<span class="deal-badge">TOP DEAL</span>${p.rating >= 4.8 ? '<span class="hot-badge">HOT</span>' : ''}</div>
      <div class="deal-info">
        <div class="deal-title">${p.title}</div>
        <div class="deal-price">${p.price || 'Contact'}</div>
        <div class="deal-moq">${p.moq || 'Flexible MOQ'}</div>
        <div class="deal-sold"><i class="fas fa-star" style="color:#f59e0b;"></i> ${p.rating||'4.5'} <span style="margin:0 0.5rem;">|</span> <i class="fas fa-shopping-cart"></i> ${p.reviews||0} orders</div>
...
function orderNow(id, title) {
  ...
  window.open(`https://wa.me/${phone}?text=${encodeURIComponent('🛒 Interested in: ' + title + ' (ID:' + id + ')')}`, '_blank');
}

document.querySelectorAll('.filter-btn').forEach(btn => { btn.addEventListener('click', function() { const cat = this.dataset.category; location.href = cat === 'all' ? 'products.html' : 'products.html?category=' + cat; }); });
document.getElementById('productSearchBtn')?.addEventListener('click', () => { const q = document.getElementById('productSearchInput').value.trim(); if(q) location.href = 'products.html?search=' + encodeURIComponent(q); });
...
window.goToDetail=goToDetail;window.orderNow=orderNow;
